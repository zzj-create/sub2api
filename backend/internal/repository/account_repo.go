...
// SetRateLimitedIfInactive atomically starts a new rate-limit generation
// without moving an already-active boundary forward. Rolling quota checks can
// run concurrently against stale scheduler snapshots, so this prevents every
// request from restarting the same 24-hour cooldown.
